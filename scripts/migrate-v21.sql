-- v21: add Granola topping variants for Frozzen and modifier pages.
...
BEGIN
  ...
  IF target_category_id IS NOT NULL THEN
    ...
    IF current_id IS NULL THEN
      ...
      VALUES ('GRANOLA', target_category_id, 0.00, 10, true, 14);
    ELSE
      UPDATE pos.products
      SET price = 0.00,
          vat_rate = 10,
  ...
  IF target_category_id IS NOT NULL THEN
    ...
    FROM pos.products
    WHERE category_id = target_category_id
      AND lower(name) = lower('GRANOLA 1€')
    LIMIT 1;

    IF current_id IS NULL THEN
      INSERT INTO pos.products (name, category_id, price, vat_rate, active, sort_order)
      VALUES ('GRANOLA 1€', target_category_id, 1.00, 10, true, 15);
    ELSE
      UPDATE pos.products
      SET price = 1.00,
          vat_rate = 10,
  ...
  IF target_category_id IS NOT NULL THEN
    ...
    FROM pos.products
    WHERE category_id = target_category_id
      AND lower(name) = lower('GRANOLA 0.5€')
    LIMIT 1;

    IF current_id IS NULL THEN
      INSERT INTO pos.products (name, category_id, price, vat_rate, active, sort_order)
      VALUES ('GRANOLA 0.5€', target_category_id, 0.50, 10, true, 14);
    ELSE
      UPDATE pos.products
      SET price = 0.50,
          vat_rate = 10,
