-- v30: safe card post-sale workflow, rectifying invoices and employee permissions.
...
SET can_post_sale_lookup = true,
    can_refund_sales = true
WHERE role = 'admin';

ALTER TABLE pos.orders
ADD COLUMN IF NOT EXISTS card_payment_status VARCHAR(24) NOT NULL DEFAULT 'not_applicable';

ALTER TABLE pos.orders
ADD COLUMN IF NOT EXISTS payment_attempt_id UUID;

ALTER TABLE pos.orders
ADD COLUMN IF NOT EXISTS card_payment_error TEXT;

ALTER TABLE pos.orders DROP CONSTRAINT IF EXISTS orders_card_payment_status_check;
ALTER TABLE pos.orders
ADD CONSTRAINT orders_card_payment_status_check
CHECK (card_payment_status IN ('not_applicable', 'awaiting', 'approved', 'failed', 'unknown'));

CREATE UNIQUE INDEX IF NOT EXISTS idx_orders_payment_attempt_id
ON pos.orders(payment_attempt_id)
WHERE payment_attempt_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_orders_card_payment_status
ON pos.orders(card_payment_status, created_at DESC);

UPDATE pos.orders
SET card_payment_status = 'approved'
WHERE payment_method = 'card'
  AND invoice_number IS NOT NULL
  AND card_payment_status = 'not_applicable';
...
CREATE TABLE IF NOT EXISTS pos.refunds (
  ...
  status VARCHAR(24) NOT NULL DEFAULT 'processing'
    CHECK (status IN ('processing', 'completed', 'failed', 'pending_verification')),
