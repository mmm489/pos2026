...
-- Each row is one HTTP call from the bridge to the card provider for sale/refund/
...
CREATE TABLE IF NOT EXISTS pos.card_transactions (
  ...
  amount NUMERIC(10,2),
  reference VARCHAR(40),                  -- provider reference
  ...
  error_message TEXT,
  request JSONB,                          -- raw payload sent to the card provider
